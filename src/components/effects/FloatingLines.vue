...
const setup = () => {
  ...
  if (ro && containerRef.value) ro.observe(containerRef.value);
  ...
  const handlePointerMove = (event: PointerEvent) => {
    ...
    if (props.parallax) {
      ...
      targetParallaxRef.value.set(
        ((x - cx) / rect.width)  *  props.parallaxStrength,
        ((y - cy) / rect.height) * -props.parallaxStrength
      );
    }
  };

  const handlePointerLeave = () => { targetInfluenceRef.value = 0.0; };
