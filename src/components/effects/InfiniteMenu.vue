...
import SuarecLogo from '../SuarecLogo.vue';
...
const emit = defineEmits<{
  itemClick: [item: InfiniteMenuItem]
}>();
...
class InfiniteGridMenu {
  ...
  private discInstances: { matricesArray: Float32Array; matrices: Float32Array[]; buffer: WebGLBuffer | null } = {
    matricesArray: new Float32Array(0), matrices: [], buffer: null
  };
  ...
  private init() {
    ...
    if (!this.discProgram) throw new Error('Failed to create shader program');
  ...
  private initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        try { ctx.drawImage(img, x, y, cellSize, cellSize); } catch { /* skip */ }
      });
      if (this.gl && this.tex) {
  ...
  private render() {
    ...
    this.gl.uniformMatrix4fv(this.discLocations.uWorldMatrix as WebGLUniformLocation | null, false, this.worldMatrix);
    this.gl.uniformMatrix4fv(this.discLocations.uViewMatrix as WebGLUniformLocation | null, false, this.camera.matrices.view);
    this.gl.uniformMatrix4fv(this.discLocations.uProjectionMatrix as WebGLUniformLocation | null, false, this.camera.matrices.projection);
    this.gl.uniform3f(this.discLocations.uCameraPosition as WebGLUniformLocation | null, this.camera.position[0], this.camera.position[1], this.camera.position[2]);
    this.gl.uniform4f(this.discLocations.uRotationAxisVelocity as WebGLUniformLocation | null,
      this.control.rotationAxis[0], this.control.rotationAxis[1], this.control.rotationAxis[2],
      this.smoothRotationVelocity * 1.1
    );

    const itemCountLoc = this.discLocations.uItemCount as WebGLUniformLocation | null;
    if (itemCountLoc !== null) this.gl.uniform1i(itemCountLoc, this.items.length);
    const atlasSizeLoc = this.discLocations.uAtlasSize as WebGLUniformLocation | null;
    if (atlasSizeLoc !== null) this.gl.uniform1i(atlasSizeLoc, this.atlasSize);
    const framesLoc = this.discLocations.uFrames as WebGLUniformLocation | null;
    if (framesLoc !== null) this.gl.uniform1f(framesLoc, this.frames);
    const scaleFactorLoc = this.discLocations.uScaleFactor as WebGLUniformLocation | null;
    if (scaleFactorLoc !== null) this.gl.uniform1f(scaleFactorLoc, this.scaleFactor);
    const texLoc = this.discLocations.uTex as WebGLUniformLocation | null;
    if (texLoc !== null) this.gl.uniform1i(texLoc, 0);

    this.gl.activeTexture(this.gl.TEXTURE0);
    this.gl.bindTexture(this.gl.TEXTURE_2D, this.tex);
    this.gl.bindVertexArray(this.discVAO);
    this.gl.drawElementsInstanced(this.gl.TRIANGLES, (this.discBuffers.indices as Uint16Array).length, this.gl.UNSIGNED_SHORT, 0, this.DISC_INSTANCE_COUNT);
  }
...
const handleActiveItem = (index: number) => {
  ...
  activeItem.value = items[index % items.length]!;
};
...
onMounted(() => {
  ...
  try {
    ...
    const handleResize = () => infiniteMenu?.resize();
...
watch(() => props.items, () => {
  if (infiniteMenu && canvasRef.value) {
    infiniteMenu.destroy();
    infiniteMenu = new InfiniteGridMenu(canvasRef.value, resolvedItems.value, handleActiveItem, moving => { isMoving.value = moving; }, menu => menu.run(), props.scale);
  }
}, { deep: true });
...
<template>
  <div class="infinite-menu-root">
    ...
    <template v-if="activeItem">
      <h2
        :class="['item-title', isMoving ? 'hidden' : 'visible']"
      >
        <SuarecLogo v-if="activeItem.title === 'SUAREC'" class="suarec-logo-title" />
        <template v-else>{{ activeItem.title }}</template>
      </h2>

      <p
        :class="['item-description', isMoving ? 'hidden' : 'visible']"
      >
      ...
      <button
        ...
        :class="['item-cta', isMoving ? 'hidden' : 'visible']"
        :style="{ bottom: isMoving ? '-80px' : '61px', transform: isMoving ? 'translateX(-50%) scale(0)' : 'translateX(-50%) scale(1)' }"
      >
        <span>&#x2197;</span>
      </button>
    </template>
...
.suarec-logo-title {
  height: calc(clamp(2.5rem, 5vw, 4rem) - 5px);
  width: auto;
  display: block;
}
